...
parser.add_argument('--dataset', type=str, default="MNIST", # CIFAR10
                    help="dataset")
parser.add_argument('--n_iter', type=int, default=30, #30
...
def train_SGD_one_iter(model, optimizer, train_loader, loss_fn, model_opt):
    model.train()
    ...
    for images, labels in train_loader:
        ...
        #-------- new
        model_opt.eval()
        with torch.no_grad():  # don't update gradients for optimal model
            yhat_optimal = model_opt(images)              # compute predicted y for optimal model
            loss_optimal = loss_fn(yhat_optimal, labels)      # calculate optimal loss

        # Update loss as P(w) - P(w*)
        adjusted_loss = loss_iter - loss_optimal
        #-------- end new
        ...
        loss.update(adjusted_loss.data.item())
        acc.update(acc_iter)
    
    return loss.avg, acc.avg

def train_SVRG_one_iter(model_k, model_snapshot, optimizer_inner, optimizer_snapshot, train_loader, snapshot_loader, loss_fn, model_opt, open=1):
    model_k.train()
...
if __name__ == "__main__":
    ...
    # initial_model_snapshot_state = model_snapshot.state_dict()



    for iteration in range(n_iter):
        t0 = time.time()
        train_loss, train_acc = train_SVRG_one_iter(model, model_snapshot, optimizer, optimizer_snapshot, train_loader, snapshot_loader, loss_fn, model, 0)
    ...
    for iteration in range(n_iter):
        ...
        # Training 
        if args.optimizer == "SGD":
            train_loss, train_acc = train_SGD_one_iter(model, optimizer, train_loader, loss_fn, model_opt)
        elif args.optimizer == "SVRG":
            train_loss, train_acc = train_SVRG_one_iter(model, model_snapshot, optimizer, optimizer_snapshot, train_loader, snapshot_loader, loss_fn, model_opt)
        else:
            raise ValueError("Unknown optimizer")
